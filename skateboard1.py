Python 2.7.3(
    default, Apr 10 2012, 23: 31: 26)[MSC v.1500 32 bit(Intel)] on win32
...
# Decorator to push speed value to ESC as soon as when changed@ speed.setter
...
self.wii.led = 0
time.sleep(0.5)# Let 's hope I don'
t activate this whilst on the board and die from this half second delay
...
def motor_off(self):
    global stop_val
stop_val = True# Causes main thread loop to stop working and speed to
default
...
def main(): #Class instance and program run
skate = Skateboard()
skate.blinky(20, 0.05)
skate.connection_process()# Wiimote checker thread
